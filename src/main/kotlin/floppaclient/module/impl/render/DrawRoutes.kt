...
object DrawRoutes : Module(
    ...
    private val acLineWidth =
        NumberSetting("Clip Line Width", 3.0, 0.1, 10.0, 0.1, description = "Line width for Auto Clip routes.")
    private val acStartColor = ColorSetting(
        "Clip Start",
        Color(255, 0, 0),
        true,
        description = "Color for the start block of Auto Clip routes."
    )
    private val acTargetColor = ColorSetting(
        "Clip Target",
        Color(0, 255, 0),
        true,
        description = "Color for the final and intermediate target blocks of Auto Clip routes."
    )
    private val acStepColor = ColorSetting(
        "Clip Step",
        Color(0, 255, 255),
        true,
        description = "Color for automatically created intermediate steps of Auto Clip routes."
    )
    private val acPathColor = ColorSetting(
        "Clip Path",
        Color(0, 0, 255),
        true,
        description = "Color for the path that the Auto Clip route follows."
    )
    private val etherLineWidth =
        NumberSetting("Ether Line Width", 3.0, 0.1, 10.0, 0.1, description = "Line width for Etherwarp routes.")
    private val etherStartColor = ColorSetting(
        "Ether Start",
        Color(255, 115, 0),
        true,
        description = "Color for the start block of Etherwarp routes."
    )
    private val etherTargetColor = ColorSetting(
        "Ether Target",
        Color(0, 82, 75),
        true,
        description = "Color for the target Block of Etberwarp routes."
    )
    private val etherPathColor = ColorSetting(
        "Ether Path",
        Color(255, 0, 255),
        true,
        description = "Color for the path that the Etherwarp route follows."
    )
    private val cmdLineWidth =
        NumberSetting("Cmd Line Width", 3.0, 0.1, 10.0, 0.1, description = "Line width for command points.")
    private val cmdPointColor =
        ColorSetting("Cmd Point", Color(255, 255, 0), true, description = "Color for the command points.")
    ...
    private fun drawRoute(key: MutableList<Int>, route: MutableList<Double>) {
        ...
        for (j in 0 until route.size / 3) {
            ...
            WorldRenderUtils.drawLine(
                ...
                starPos.yCoord,
                starPos.zCoord,
                starPos.xCoord + path.xCoord,
                starPos.yCoord + path.yCoord,
                starPos.zCoord + path.zCoord,
                acPathColor.value,
                acLineWidth.value.toFloat()
            )
            ...
            if (direction[0] > dist) {
                for (ii in 1..floor(abs(direction[0] / dist)).toInt()) {
                    ...
                    WorldRenderUtils.drawBoxAtBlock(
                        starPos.xCoord - sinDeg(yaw) * cosDeg(pitch) * dist * ii - 0.5,
                        starPos.yCoord - sinDeg(pitch) * dist * ii - 1,
                        starPos.zCoord + cosDeg(yaw) * cosDeg(pitch) * dist * ii - 0.5,
                        acStepColor.value,
                        acLineWidth.value.toFloat()
                    )
                }
            ...
            WorldRenderUtils.drawBoxAtBlock(
                starPos.xCoord + path.xCoord - 0.5,
    ...
    private fun drawEther(key: MutableList<Int>, value: BlockPos) {
        ...
        WorldRenderUtils.drawBoxAtBlock(
            ...
            start.yCoord,
            start.zCoord,
            etherStartColor.value,
            etherLineWidth.value.toFloat()
        )
...
//        target = target.addVector(DataHandler.isNegative(target.xCoord), 0.0, DataHandler.isNegative(target.zCoord))
        ...
        // Draw the line
        WorldRenderUtils.drawLine(
            starPos.xCoord, starPos.yCoord, starPos.zCoord, targetPos.xCoord,
