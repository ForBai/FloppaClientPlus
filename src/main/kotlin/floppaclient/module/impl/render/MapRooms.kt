...
object MapRooms : Module(
    ...
    val mapRoomNames = StringSelectorSetting(
        "Room Names",
        "Puzzles / Trap",
        arrayListOf("None", "Puzzles / Trap", "All"),
        description = "Shows names of rooms on map."
    )
    val mapRoomSecrets = StringSelectorSetting(
        "Room Secrets",
        "Off",
        arrayListOf("Off", "On", "Replace Checkmark"),
        description = "Shows total secrets of rooms on map. REPLACE CHECKMARKS NOT ADDED!"
    )
    val mapColorText =
        BooleanSetting("Color Text", false, description = "Colors name and secret count based on room state.")
    val mapCheckmark = StringSelectorSetting(
        "Room Checkmarks",
        "Default",
        arrayListOf("None", "Default", "NEU"),
        description = "Adds room checkmarks based on room state."
    )
    val mapRoomTransparency = NumberSetting("Room Opacity", 1.0, 0.0, 1.0, 0.01)
    val mapDarkenPercent =
        NumberSetting("DarkenMultiplier", 0.4, 0.0, 1.0, 0.01, description = "How much to darken undiscovered rooms")
