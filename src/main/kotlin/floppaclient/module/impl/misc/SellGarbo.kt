...
object SellGarbo : Module(
    ...
    private val sleep: Double by NumberSetting(
        "Sleep",
        200.0,
        10.0,
        1000.0,
        10.0,
        description = "Delay in between clicks."
    )
    private val onlyTrades: Boolean by BooleanSetting(
        "Only Trades",
        true,
        description = "Only activates in the trades menu. If disabled auto sell will also work with all npc shops."
    )

    private val dungeonItems: Boolean by BooleanSetting(
        "Dugeon Mob Drops",
        true,
        description = "Will auto sell Dungeon mob drops."
    )
    private val sellSuperBoom: Boolean by BooleanSetting(
        "Sell SuperBoom",
        false,
        description = "Will also sell SuperBoom tnt."
    )
    private val sellRevStone: Boolean by BooleanSetting(
        "Sell Revive Stone",
        false,
        description = "Will also sell Revive stones."
    )
    private val sellMaxStat: Boolean by BooleanSetting(
        "Sell max stat",
        false,
        description = "Will also sell items with +50% stat boost."
    )
    private val sellStarred: Boolean by BooleanSetting(
        "Sell Starred",
        false,
        description = "When enabled starred items will also be sold."
    )
    private val sellRecombed: Boolean by BooleanSetting(
        "Sell Recombed",
        false,
        description = "When enabled rarity upgraded items will also be sold."
    )
    private val treasureTalis: Boolean by BooleanSetting(
        "Treasure Talis",
        false,
        description = "Also sells treasure talisman."
    )
    private val other: String by StringSetting(
        "Other",
        "",
        100,
    ...
    private val message: Boolean by BooleanSetting(
        "Message on Finish",
        true,
        description = "Puts a message in chat when it is done."
    )
    private val clickMethod = +StringSelectorSetting(
        "Click Type",
        "Shift Click",
        arrayListOf("Left Click", "Shift Click", "Middle Click"),
    ...
    fun onRender(event: GuiScreenEvent.BackgroundDrawnEvent) {
        ...
        if (stack.displayName == "§aSell Item" || (stack.lore.isNotEmpty() && stack.lore.last() == "§eClick to buyback!")) inSellMenu =
            true
        else {
            inSellMenu = false
            return
