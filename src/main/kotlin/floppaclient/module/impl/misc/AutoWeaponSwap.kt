...
object AutoWeaponSwap : Module(
    ...
    private val axeOfTheShredded =
        BooleanSetting("AOTS Swap", false, description = "Include Axe of the Shredded in the weapon swap cycle.")
    private val soulWhip =
        BooleanSetting("Soul Whip Swap", false, description = "Include Soul Whip in the weapon swap cycle.")
    private val terminator =
        BooleanSetting("Terminator Swap", false, description = "Include Terminator in the weapon swap cycle.")
    private val iceSpray =
        BooleanSetting("Ice Spray Swap", false, description = "Include Ice Spray in the weapon swap cycle.")
    private val termSleep = NumberSetting(
        "Sleep ms",
        50.0,
        10.0,
        100.0,
        5.0,
        description = "Delay between Terminator clicks. This will determine the CPS on the Terminator and lets it exceed your left click CPS."
    )
        .withDependency { this.terminator.enabled }
    private val fromInv = BooleanSetting(
        "From Inv",
        false,
        description = "Lets you use Soul Whip, AOTS and Ice Spray from inventory. §cNot recommended."
    )

    private val leftClickItems = setOf("suspicious", "fabled", "heroic", "spicy", "withered")
    private val leftClickBlacklist =
        arrayOf(SkyblockItem.AOTV, SkyblockItem.JERRY_GUN, SkyblockItem.BONZO_STAFF, SkyblockItem.BONZO_STAFF_FRAGGED)
