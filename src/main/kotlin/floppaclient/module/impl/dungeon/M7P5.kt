...
object M7P5 : Module(
    ...
    private val statueBox =
        BooleanSetting("Statue Box", true, description = "Renders a box, showing where you can kill a dragon.")
    private val boxThickness =
        NumberSetting("Box Thickness", 2.5, 0.1, 10.0, 0.1, description = "Thickness of the Statue Box.")
            .withDependency { this.statueBox.enabled }
    private val dragonSpawnTimer =
        BooleanSetting("Dragon Spawn Timer", true, description = "Renders a timer for when a dragon spawns in.")
    ...
    fun onWorldRender(event: RenderWorldLastEvent) {
        ...
        if (M7Drags.GREEN.isAlive) drawCustomSizedBoxAt(
            7.0,
            8.0,
            80.0,
            30.0,
            ...
            30.0,
            Color(0, 255, 0),
            boxThickness.value.toFloat(),
            false
        )
        if (M7Drags.RED.isAlive) drawCustomSizedBoxAt(
            14.5,
            13.0,
            45.5,
            25.0,
        ...
        if (M7Drags.ORANGE.isAlive) drawCustomSizedBoxAt(
            72.0,
            7.0,
            47.0,
            30.0,
        ...
        if (M7Drags.BLUE.isAlive) drawCustomSizedBoxAt(
            71.5,
            16.0,
            82.5,
            25.0,
        ...
        if (M7Drags.PURPLE.isAlive) drawCustomSizedBoxAt(
            45.5,
            13.0,
            113.5,
            23.0,
    ...
    fun onParticle(event: ReceivePacketEvent) {
        ...
        if (packet.particleType.equals(EnumParticleTypes.FLAME)) {
            val drags = M7Drags.values()
                .find { it.particles.xCoord == packet.xCoordinate && it.particles.zCoord == packet.zCoordinate }
                ?: return
