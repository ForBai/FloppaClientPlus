...
object ClipTools {
    ...
    private fun getDelay(index: Int): Long {
        return longArrayOf(
            0,
            ClipSettings.clipDelay1.value.toLong(),
            ClipSettings.clipDelay2.value.toLong(),
            ClipSettings.clipDelay3.value.toLong(),
            ClipSettings.clipDelay4.value.toLong(),
            ClipSettings.clipDelay5.value.toLong(),
            ClipSettings.clipDelay6.value.toLong(),
            ClipSettings.clipDelay7.value.toLong(),
            ClipSettings.clipDelay8.value.toLong(),
            ClipSettings.clipDelay9.value.toLong()
        )[index]
    }

    fun dClip(dist: Double, yaw: Float = yaw(), pitch: Float = 0.0f) {
        clip(
            -GeometryUtils.sinDeg(yaw) * GeometryUtils.cosDeg(pitch) * dist,
    ...
    private fun farclip(
        distTotal: Double,
        yaw: Double = yaw().toDouble(),
        pitch: Double = GeometryUtils.pitch().toDouble(),
        startii: Int = 0,
        delayOffs: Long = 0
    ): Int {
    ...
    private fun farclip(
        distTotal: Double,
        yaw: Float = yaw(),
        pitch: Float = GeometryUtils.pitch(),
        startii: Int = 0,
        delayOffs: Long = 0
    ): Int { //directional clip towards where you are facing in 3D
        val dist = ClipSettings.baseClipDistance.value * sign(distTotal)
        val n = floor(abs(distTotal / dist)).toInt()
//        val timer = Timer()
        if (startii + n > 10) {
            modMessage(
                "§cError: specified distance exceeds max set distance of "
        ...
        if (n > 0 && startii == 0) {
            if (delayOffs == 0L) {
                dClip(
                    ...
                    yaw,
                    pitch
                ) // first clip without set timeout required to make it faster, setTimeout seems to add about 40ms delay when 0ms are specified
            } else {
     ...
     */
    fun executeClipRoute(
        route: MutableList<Double>,
        rotation: Int = 0,
        startDelay: Int = 0,
